...
import { App, Button, Drawer, Space, Switch, Tag, Typography, message } from 'antd';
...
export function SchedulerPage() {
  ...
  function confirmDelete(row: TaskRow) {
    modal.confirm({
      ...
      onOk: async () => { await deleteTask(row.id); message.success('已删除'); actionRef.current?.reload(); },
    });
  }
    ...
    <div style={{ padding: '0 0 24px' }}>
      <ProTable<TaskRow>
        ...
        request={async (params) => {
          const res = await listTasks({ keyword: params.keyword, page: params.current, page_size: params.pageSize });
        ...
        headerTitle="定时任务"
        toolBarRender={() => [
          <Permission code="settings:update" key="add">
      ...
      >
        ...
        <ProFormText name="cron_expr" label="Cron 表达式" placeholder="0 * * * *" />
