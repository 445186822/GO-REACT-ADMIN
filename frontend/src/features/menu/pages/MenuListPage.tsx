...
import {
  ModalForm, ProFormDigit, ProFormSelect, ProFormText,
...
type FlatMenuRow = MenuTreeRow & { depth: number };
...
export function MenuListPage() {
  ...
  const columns: ProColumns<MenuTreeRow>[] = [
    ...
    { title: '类型', dataIndex: 'type', width: 100, render: (_, row) => <Tag color={typeColor(row.type)}>{typeText(row.type)}</Tag> },
    ...
    {
      title: '操作', ...operationColumnProps<MenuTreeRow>(240),
      render: (_, row) => (
        <Space wrap={false} className="table-action-buttons">
          {row.type !== 'button' && (
            <Permission code="menu:create">
              <Button type="link" size="small" icon={<PlusOutlined />} onClick={() => openCreate(row)}>子项</Button>
            </Permission>
          )}
  ...
  function openCreate(parent?: MenuRow) {
    setEditing(null);
    setFormSeed({ parent_id: parent?.id, type: parent ? 'button' : 'page', sort_order: parent ? parent.sort_order + 1 : 0 });
    setOpen(true);
  }

  function openEdit(row: MenuRow) {
    setEditing(row);
    setFormSeed({ parent_id: row.parent_id ?? undefined, type: row.type, code: row.code, name: row.name, path: row.path ?? undefined, component: row.component ?? undefined, icon: row.icon ?? undefined, sort_order: row.sort_order });
    setOpen(true);
  }
  ...
  function confirmDelete(row: MenuRow) {
    modal.confirm({
      ...
      onOk: async () => { await deleteMenu(row.id); message.success('菜单已删除'); actionRef.current?.reload(); },
    });
  }

  async function submit(values: MenuForm) {
    const payload: MenuForm = { ...values, parent_id: values.parent_id || null, path: values.path || null, component: values.component || null, icon: values.icon || null, sort_order: values.sort_order ?? 0 };
    try {
      if (editing) await updateMenu(editing.id, payload);
      else await createMenu(payload);
      message.success(editing ? '菜单已更新' : '菜单已创建');
      setOpen(false); setEditing(null);
      actionRef.current?.reload();
      return true;
    } catch { message.error('保存菜单失败，请检查编码是否重复或父级是否合法'); return false; }
  }

  function getMenuActionMenuItems(row: MenuTreeRow): MenuProps['items'] {
    const items: MenuProps['items'] = [];
    if (row.type !== 'button' && hasPermission('menu:create')) items.push({ key: 'child', icon: <PlusOutlined />, label: '子项', onClick: () => openCreate(row) });
    if (hasPermission('menu:update')) items.push({ key: 'edit', icon: <EditOutlined />, label: '编辑', onClick: () => openEdit(row) });
    if (hasPermission('menu:delete')) items.push({ key: 'delete', icon: <DeleteOutlined />, label: '删除', danger: true, onClick: () => confirmDelete(row) });
    return items;
  }

  function renderMobileActions(row: MenuTreeRow) {
  ...
  async function exportMenus() {
    ...
    await exportExcel<MenuRow>('menus.xlsx', 'Menus', [
      { title: 'ID', dataIndex: 'id' }, { title: '父级 ID', dataIndex: 'parent_id' },
      { title: '名称', dataIndex: 'name' }, { title: '类型', dataIndex: 'type', render: (_, row) => typeText(row.type) },
      { title: '权限编码', dataIndex: 'code' }, { title: '路由', dataIndex: 'path' },
      { title: '组件', dataIndex: 'component' }, { title: '图标', dataIndex: 'icon' }, { title: '排序', dataIndex: 'sort_order' },
    ], rows);
  ...
  return (
    <div>
      <ResponsiveProTable<MenuTreeRow>
        actionRef={actionRef}
        rowKey="id"
        columns={columns}
        search={false}
        ...
        toolBarRender={() => [
          ...
          <ExportButton key="export" onClick={exportMenus}>导出 Excel</ExportButton>,
        ]}
      />

      <ModalForm<MenuForm>
        key={editing ? `edit-${editing.id}` : `create-${formSeed.parent_id ?? 'root'}-${formSeed.type ?? 'page'}`}
        title={editing ? '编辑菜单' : '新增菜单'} open={open}
        modalProps={{ destroyOnHidden: true, onCancel: () => { setOpen(false); setEditing(null); } }}
        initialValues={formSeed} onFinish={submit}
      >
        <ProFormSelect name="parent_id" label="父级菜单" options={parentOptions} placeholder="不选择则为根菜单"
          fieldProps={{ allowClear: true, showSearch: true, optionFilterProp: 'label' }} />
        <ProFormSelect name="type" label="类型" rules={[{ required: true }]}
          options={[{ label: '目录', value: 'directory' }, { label: '页面', value: 'page' }, { label: '按钮', value: 'button' }]} />
        ...
        <ProFormSelect name="icon" label="图标" options={menuIconOptions} placeholder="请选择菜单图标"
          fieldProps={{ allowClear: true, showSearch: true, virtual: false, listHeight: 360, popupClassName: 'menu-icon-select-popup', optionFilterProp: 'searchLabel', optionRender: (option: any) => renderMenuIconOption(option.value) }} />
