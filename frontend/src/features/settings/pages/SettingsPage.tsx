...
import {
  ModalForm, ProFormSelect, ProFormSwitch, ProFormText, ProFormTextArea,
...
export function SettingsPage() {
  ...
  const columns: ProColumns<SettingRow>[] = [
    ...
    {
      title: '配置值', dataIndex: 'setting_value', search: false, ellipsis: true,
    ...
    {
      title: '操作', ...operationColumnProps<SettingRow>(120),
      render: (_, row) => (
  ...
  function openEdit(row: SettingRow) { setEditing(row); setOpen(true); }
  ...
  async function exportSettings() {
    try {
      await exportExcel<SettingRow>('settings.xlsx', 'Settings', [
        { title: 'ID', dataIndex: 'id' }, { title: '分组', dataIndex: 'group_key' },
        { title: '键', dataIndex: 'setting_key' }, { title: '值', dataIndex: 'setting_value' },
        { title: '类型', dataIndex: 'value_type' }, { title: '说明', dataIndex: 'description' },
        { title: '加密', dataIndex: 'is_encrypted' }, { title: '更新时间', dataIndex: 'updated_at' },
      ], rows);
    ...
    <div className="settings-workspace">
      <div className="settings-header">
        <Space>
          ...
          <Permission code="settings:update">
            <Button type="primary" icon={<PlusOutlined />} onClick={() => { setEditing(null); setOpen(true); }}>新增配置</Button>
          </Permission>
        </Space>
      </div>

      <Tabs className="settings-group-tabs" activeKey={active?.key} onChange={setActiveGroup}
          ...
          children: (
            <>
              <div className="settings-group-summary">
                <div><Text strong>{group.title}</Text><br /><Text type="secondary">{group.description}</Text></div>
              </div>
              <ResponsiveProTable<SettingRow>
                ...
                renderMobileActions={(row) => (
                  <Permission code="settings:update">
                    <Button type="text" size="small" icon={<EditOutlined />} onClick={() => openEdit(row)} aria-label="编辑" />
                  </Permission>
                )}
              />
            </>
      ...
      <ModalForm<SettingForm>
        title={editing ? '编辑配置' : '新增配置'} open={open}
        modalProps={{ destroyOnHidden: true, onCancel: () => { setOpen(false); setEditing(null); } }}
        initialValues={editing ? {
          group_key: editing.group_key, setting_key: editing.setting_key,
          setting_value: editing.setting_value, value_type: editing.value_type,
          description: editing.description ?? undefined, is_encrypted: editing.is_encrypted,
        } : { group_key: activeGroup || 'system', value_type: 'string', is_encrypted: false }}
        onFinish={submit}
      >
        <ProFormSelect name="group_key" label="分组" rules={[{ required: true }]}
          options={[
            { label: '基础信息', value: 'system' }, { label: '安全策略', value: 'security' },
            { label: '文件与存储', value: 'file' }, { label: '通知设置', value: 'notification' }, { label: 'AI 设置', value: 'ai' },
          ]}
        />
        ...
        <ProFormSelect name="value_type" label="类型"
          options={[{ label: 'string', value: 'string' }, { label: 'number', value: 'number' }, { label: 'boolean', value: 'boolean' }, { label: 'json', value: 'json' }]}
        />
